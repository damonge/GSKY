...
class GSKYCore(object):
    ...
    def __call__(self, ctx):
        ...
        try:
            ...
            gskytheor = GSKYTheory(saccfile, HMparams, cosmo)
            ...
            for tr_i, tr_j in self.trc_combs:
                ...
                if self.ells != 'NONE':
                    if self.conv_win:
                        # Get window
                        win = saccfile.get_tag('window', tracers=(tr_i, tr_j), data_type=datatype)
                        if type(win) is list:
                            win = win[0]
                    ...
                    ells_curr = np.array(saccfile.get_tag('ell', tracers=(tr_i, tr_j), data_type=datatype))
                    if self.conv_win:
                        # Get window
                        win = saccfile.get_tag('window', tracers=(tr_i, tr_j), data_type=datatype)
                        if type(win) is list:
                ...
                indx = saccfile.indices(datatype, (tr_i, tr_j))
