...
from gsky.cov_psf_from_mocks_fourth_moment import CovPSFFromMocksFourthMoment
...
class CovPSFMocksFourthMoment(PipelineStage):
    name = "CovPSFMocksFourthMoment"
    inputs = []
    ...
    def run(self):
        ...
        """
        test = CovPSFFromMocksFourthMoment()
        cls, ells = test.go()

        np.save(self.get_output_fname('cls_signal_realiz_fourth_moment', 'npy'), cls)
        logger.info('Written signal cls to {}.'.format(self.get_output_fname('cls_signal_realiz_fourth_moment', ext='npy')))

        np.save(self.get_output_fname('l_eff_noise_fourth_moment', 'npy'), ells)
