    ...
    def run(self):
        ...
        for alg in pz_algs :
            ...
            for i,file in enumerate(patch_files):
                ...
                ids, i_g, i_p = np.intersect1d(gal_ids, pdf_ids, assume_unique=True, return_indices=True)
                #id_matches = np.isin(gal_ids, pdf_ids)
                matched[i_g] = True
                pdfs[i_g, :] = data_pdf['P(z)'][i_p]

            logger.info("%d galaxies matched with pdfs out of %d"%(np.sum(matched),len(matched)))
            ...
            col1 = fits.Column(name='object_id', format='K', array=np.array(gal_ids, dtype=int))
