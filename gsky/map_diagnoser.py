...
from .types import FitsFile, DirFile
import numpy as np
...
class MapDiagnoser(PipelineStage):
    ...
    inputs = [('masked_fraction', FitsFile),
              ...
              ('star_map', FitsFile),
              ('depth_map', FitsFile),
              ('ccdtemp_maps', FitsFile),
              ('airmass_maps', FitsFile),
              ('exptime_maps', FitsFile),
              ('skylevel_maps', FitsFile),
              ('sigma_sky_maps', FitsFile),
              ('seeing_maps', FitsFile),
              ('ellipt_maps', FitsFile),
    ...
    def compute_stats(self, ng_map, sys_map):
        ...
        mean, bin_edges, _ = binned_statistic(sys_use, ng_use,
                                              statistic='mean',
    ...
    def parse_input(self):
        ...
        if self.config['sys_collapse_type'] == 'average':
            ...
            raise ValueError('Systematic map flattening mode %s '
                             'unknown. Use \'average\' or \'median\'' %
                             (self.config['sys_collapse_type']))
    ...
    def get_mask(self):
        print("Reading mask")
        self.fsk, mp_depth = read_flat_map(self.get_input("depth_map"),
                                           i_map=0)
        ...
        fskb, self.mskfrac = read_flat_map(self.get_input("masked_fraction"),
                                           i_map=0)
        compare_infos(self.fsk, fskb)

        msk_bo = np.zeros_like(self.mskfrac)
        msk_bo[self.mskfrac > self.config['mask_thr']] = 1
    ...
    def get_sysmaps(self):
        ...
        _, self.temps['stars'] = read_flat_map(self.get_input("star_map"),
                                               i_map=0)
        for oc in ['ccdtemp', 'airmass', 'exptime', 'skylevel',
    ...
    def get_nmaps(self):
        ...
        for ib in range(nbins):
            _, self.ng['bin%d' % (ib+1)] = read_flat_map(None,
                                                         hdu=hdul[2*ib])
    ...
    def analyze_bin_systematic(self, bin_name, syst_name, bands=False):
        ...
        for i in range(nmaps):
            ...
            if nmaps != 1:
                ax[i].text(0.9, 0.9, self.bands[i],
                           transform=ax[i].transAxes)
        ax[-1].set_xlabel(self.xlabels[syst_name],
                          fontsize=16)
        f.tight_layout()
