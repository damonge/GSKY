...
class ShearMapper(PipelineStage) :
    ...
    def get_nz_cosmos(self) :
        ...
        """
        # TODO: this is wrong. We need to include lensing weights when available.
    ...
    def get_nz_stack(self, cat, codename):
        ...
        for i in range(self.nbins):
            msk_good = (cat['tomo_bin']==i) & pdfgood & cat['shear_cat']
            hz_orig = np.sum(weights[msk_good][:, None] * p[msk_good], axis=0)
    ...
    def run(self):
        ...
        mhats = np.array([head_cat['MHAT_%d'%(ibin+1)] for ibin in range(self.nbins)])
