...
class PSpecPlotter(PipelineStage) :
    ...
    inputs=[]
    outputs=[]
    config_options={'path2saccs': list, 'output_run_dir': 'NONE', 'output_plot_dir': 'NONE',
                    'fig_name': str, 'tracers': list, 'plot_comb': 'all', 'cl_type': 'cl_ee', 'plot_errors': False,
    ...
    def run(self):
        ...
        for saccdir in self.config['saccdirs']:
            path2sacc = os.path.join(self.output_dir, saccdir)
            if self.config['output_run_dir'] != 'NONE':
                path2sacc = os.path.join(path2sacc, self.config['output_run_dir'])
