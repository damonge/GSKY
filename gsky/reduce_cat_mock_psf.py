...
class ReduceCatMockPSF(PipelineStage):
    name = "ReduceCatMockPSF"
    ...
    def make_PSF_maps(self, star_cat, fsk):
        ...
        # T_I = Mxx + Myy
        e_plus_I = star_cat['e1']
        e_cross_I = star_cat['e2']
    ...
    def make_PSF_res_maps(self, star_cat, fsk):
        ...
        delta_e_plus = star_cat['de1']
        delta_e_cross = star_cat['de2']
    ...
    def run(self):
        ...
        if 'GAMA09H' in self.get_input('shape_catalog') and self.config['rm_gama09h_region']==True:
            ...
            logger.info("Good seeing removal percentage %f", (np.sum(good_seeing_mask)/len(cat)))
            logger.info("Good seeing removal number %f", np.sum(good_seeing_mask))
        ...
        if self.get_input('star_catalog') != 'NONE':
            ...
            mPSFstar = self.make_PSF_maps(star_cat, fsk)
            ...
            mPSFresstar = self.make_PSF_res_maps(star_cat, fsk)
            # logger.info("Computing w2e2.")
            ...
            mPSFstar = self.make_PSF_fourth_moment_maps(star_cat, fsk)
            # logger.info("Computing w2e2.")
            # w2e2 = self.get_w2e2(star_cat, M4_plus_I, M4_cross_I, fsk)
            logger.info("Writing output to {}.".format(self.get_output('M4_PSF_map')))
            header = fsk.wcs.to_header()
            hdus = []
            shp_mp = [fsk.ny, fsk.nx]
            # Maps
            ...
            # hdus.append(fits.BinTableHDU.from_columns(cols))
            hdulist = fits.HDUList(hdus)
            hdulist.writeto(self.get_output('M4_PSF_map'), overwrite=True)

            
            #fourth moment PSF residual - PSF stars
            logger.info('Creating M4_PSF residual maps.')
            mPSFresstar = self.make_PSF_res_fourth_moment_maps(star_cat, fsk)
            # logger.info("Computing w2e2.")
            # w2e2 = self.get_w2e2(star_cat, delta_M4_plus, delta_M4_cross, fsk)
            # Write M4_PSFres map
            logger.info("Writing output to {}.".format(self.get_output('M4_PSFres_map')))
            header = fsk.wcs.to_header()
            hdus = []
            shp_mp = [fsk.ny, fsk.nx]
            # Maps
            ...
            # hdus.append(fits.BinTableHDU.from_columns(cols))
            hdulist = fits.HDUList(hdus)
            hdulist.writeto(self.get_output('M4_PSFres_map'), overwrite=True)
        else:
